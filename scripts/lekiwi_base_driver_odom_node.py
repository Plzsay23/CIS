...
class LeKiwiBaseDriverOdom(Node):
    def __init__(self, args):
        ...
        self.get_logger().info(
            f"LeKiwi base driver with odom started. "
            f"port={self.port}, odom_frame={self.odom_frame}, base_frame={self.base_frame}, "
            f"publish_tf={self.publish_tf}, measured_wheel_velocity={self.use_measured_wheel_velocity}"
        )
    ...
    def connect_and_configure(self):
        ...
        self.get_logger().info("Configuring wheel motors only: IDs 7, 8, 9")
        ...
        try:
            ...
            self.get_logger().warn(f"Acceleration write skipped/failed: {e}")
    ...
    def on_estop(self, msg: Bool):
        ...
        if self.estop:
            self.get_logger().warn("E-stop enabled. Stopping motors.")
            self.stop_motors()
    ...
    def publish_odom(self, vx_body, vy_body, wz):
        ...
        # Large-ish covariance because this first version is dead-reckoning.
...
def parse_args():
    ...
    parser.add_argument("--max-xy", type=float, default=0.30)
