...
def clamp_twist(msg: Twist, max_x: float, max_y: float, max_yaw: float) -> Twist:
    out = Twist()
    out.linear.x = clamp(msg.linear.x, -max_x, max_x)
...
def is_nonzero(msg: Twist, eps: float = 1e-4) -> bool:
    return (
...
class CmdVelMuxNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("dashboard_timeout_sec", 0.5)
        ...
        self.get_logger().info("priority: emergency_stop > dashboard > auto > act")
