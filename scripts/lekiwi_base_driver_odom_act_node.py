...
class LeKiwiBaseDriverOdom(Node):
    def __init__(self, args):
        ...
        self.act_action_space = args.act_action_space
        self.act_arm_action_timeout_s = float(args.act_arm_action_timeout_s)
        self.act_min_runtime_s = float(args.act_min_runtime_s)
        self.act_stationary_window_s = float(args.act_stationary_window_s)
        self.act_stationary_tick_threshold = float(args.act_stationary_tick_threshold)
        self.act_timeout_s = float(args.act_timeout_s)
        self.act_publish_state_hz = float(args.act_publish_state_hz)
        self.act_started_time: Optional[float] = None
        self.act_last_action_time: Optional[float] = None
        self.act_last_state_pub_time = 0.0
        self.act_done_published = False
        self.act_position_history: Deque[tuple[float, Dict[str, int]]] = deque()
        ...
        self.act_arm_state_pub = self.create_publisher(String, "/act/arm_state", 10)
        self.act_status_pub = self.create_publisher(String, "/act/status", 10)
        self.act_done_pub = self.create_publisher(String, "/act/done", 10)
        ...
        self.act_enabled_sub = self.create_subscription(Bool, "/act/enabled", self.on_act_enabled, 10)
        self.act_arm_action_sub = self.create_subscription(String, "/act/arm_action", self.on_act_arm_action, 10)
        ...
        self.get_logger().info(
            "LeKiwi base+arm+ACT dashboard driver started. "
    ...
    def on_act_enabled(self, msg: Bool):
        ...
        now = time.monotonic()
        if self.act_enabled:
            self.act_started_time = now
            self.act_last_action_time = None
            self.act_done_published = False
            self.act_position_history.clear()
            self.get_logger().warn("/act/enabled true. Base locked for ACT phase.")
        else:
            self.act_started_time = None
            self.act_last_action_time = None
            self.act_done_published = False
            self.act_position_history.clear()
            self.get_logger().warn("/act/enabled false. Base lock released.")

        self.publish_act_status(force=True)

    def normalize_to_tick(self, name: str, value: float) -> int:
        # RANGE_M100_100 convention: -100 -> range_min, +100 -> range_max.
        lo, hi = ARM_RANGES[name]
        v = clamp(float(value), -100.0, 100.0)
        alpha = (v + 100.0) / 200.0
        return self.clamp_arm_tick(name, lo * (1.0 - alpha) + hi * alpha)

    def parse_act_action_payload(self, payload: str) -> Dict[str, float]:
        data = json.loads(payload)
        if isinstance(data, dict):
            if "raw_position" in data and isinstance(data["raw_position"], dict):
                return data["raw_position"]
            if "action" in data and isinstance(data["action"], dict):
                return data["action"]
            return data
        raise ValueError("ACT action payload must be a JSON object")

    def apply_act_arm_action(self, values: Dict[str, float]):
        raw_targets: Dict[str, int] = {}
        gripper_value = None

        # Accept both LeRobot-ish names and dashboard driver names.
        aliases = {
        ...
        for key, value in values.items():
            ...
            fv = float(value)
            if self.act_action_space == "normalized":
                raw_targets[name] = self.normalize_to_tick(name, fv)
            elif self.act_action_space == "raw":
                raw_targets[name] = self.clamp_arm_tick(name, fv)
            else:
                # auto: values in [-100, 100] are treated as normalized; otherwise raw ticks.
                if -100.0 <= fv <= 100.0:
                    raw_targets[name] = self.normalize_to_tick(name, fv)
                else:
                    raw_targets[name] = self.clamp_arm_tick(name, fv)
        ...
        if raw_targets:
            self.write_arm_positions_raw(raw_targets)

        if gripper_value is not None and "arm_gripper" in self.active_arm_names:
            # For gripper, values in [0,100] are UI normalized by convention.
            self.write_gripper_ui_norm(float(clamp(gripper_value, 0.0, 100.0)), num_retry=1)

    def on_act_arm_action(self, msg: String):
        ...
        try:
            values = self.parse_act_action_payload(msg.data)
            self.apply_act_arm_action(values)
            self.act_last_action_time = time.monotonic()
        except Exception as e:
            self.get_logger().error(f"ACT arm action failed: {e}; payload={msg.data[:300]}")

    def on_cmd_vel(self, msg: Twist):
        if self.act_enabled or self.arm_motion_busy or self.has_active_arm_jog():
            return
    ...
    def get_active_cmd(self):
        ...
        if self.estop or self.act_enabled or self.arm_motion_busy or self.has_active_arm_jog(now):
            return 0.0, 0.0, 0.0
    ...
    def publish_act_done_once(self, result: str, reason: str, extra: Optional[dict] = None):
        if self.act_done_published:
            return
        self.act_done_published = True
        payload = {
            "result": result,
            "reason": reason,
            "time": time.time(),
            "elapsed_s": None if self.act_started_time is None else time.monotonic() - self.act_started_time,
        }
        if extra:
            payload.update(extra)
        msg = String()
        msg.data = json.dumps(payload, ensure_ascii=False)
        self.act_done_pub.publish(msg)
        self.get_logger().warn(f"ACT done: {msg.data}")

    def compute_act_stationary(self, now: float, current: Dict[str, int]):
        self.act_position_history.append((now, dict(current)))
        while self.act_position_history and now - self.act_position_history[0][0] > self.act_stationary_window_s:
            self.act_position_history.popleft()

        if len(self.act_position_history) < 2:
            return False, 0.0

        names = list(current.keys())
        max_delta = 0.0
        for name in names:
            vals = [row[1].get(name, current[name]) for row in self.act_position_history]
            if vals:
                max_delta = max(max_delta, float(max(vals) - min(vals)))

        stationary = max_delta <= self.act_stationary_tick_threshold
        return stationary, max_delta

    def publish_act_status(self, force: bool = False):
        now = time.monotonic()
        if not force and self.act_publish_state_hz > 0:
            min_dt = 1.0 / self.act_publish_state_hz
            if now - self.act_last_state_pub_time < min_dt:
                return
        self.act_last_state_pub_time = now

        raw_position = {}
        stationary = False
        max_delta = None
        try:
            raw_position = self.read_arm_positions_raw()
            if self.act_enabled:
                stationary, max_delta = self.compute_act_stationary(now, raw_position)
        except Exception as e:
            self.get_logger().warn(f"ACT arm state read failed: {e}")

        elapsed = None if self.act_started_time is None else now - self.act_started_time
        since_action = None if self.act_last_action_time is None else now - self.act_last_action_time

        status = {
            "enabled": self.act_enabled,
            "elapsed_s": elapsed,
            "since_last_action_s": since_action,
            "stationary": stationary,
            "stationary_max_delta_ticks": max_delta,
        ...
        self.act_status_pub.publish(msg)
        self.act_arm_state_pub.publish(msg)

        if not self.act_enabled or self.act_done_published or elapsed is None:
            return

        if elapsed >= self.act_timeout_s:
            self.publish_act_done_once("timeout", "act_timeout", status)
            return

        if elapsed >= self.act_min_runtime_s and stationary and since_action is not None:
            if since_action >= self.act_stationary_window_s:
                self.publish_act_done_once("success", "arm_stationary", status)
    ...
    def on_timer(self):
        ...
        try:
            self.publish_act_status(force=False)
        except Exception as e:
            self.get_logger().error(f"ACT status publish failed: {e}")
...
def parse_args():
    ...
    parser.add_argument("--act-action-space", choices=["auto", "raw", "normalized"], default="auto")
    parser.add_argument("--act-arm-action-timeout-s", type=float, default=0.7)
    parser.add_argument("--act-min-runtime-s", type=float, default=3.0)
    parser.add_argument("--act-stationary-window-s", type=float, default=2.0)
    parser.add_argument("--act-stationary-tick-threshold", type=float, default=5.0)
    parser.add_argument("--act-timeout-s", type=float, default=18.0)
    parser.add_argument("--act-publish-state-hz", type=float, default=10.0)
