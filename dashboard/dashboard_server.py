...
async def index():
    if not DEFAULT_HTML_PATH.exists():
        return HTMLResponse(
            "<h1>dummy_dashboard.html not found</h1>"
            "<p>Put dummy_dashboard.html in the same directory as dashboard_server.py.</p>",
            status_code=404,
        )
...
async def health():
    return {
        ...
        "html": str(DEFAULT_HTML_PATH),
        "cmd_topic": "/dashboard/cmd_vel",
        "estop_topic": "/emergency_stop",
        "arm_cmd_topic": "/dashboard/arm_cmd",
        "allowed_arm_commands": sorted(ALLOWED_ARM_COMMANDS),
        "limits": {
