...
class YoloWristCocoProxyDetector(Node):
    def __init__(self, args):
        super().__init__("yolo_wrist_coco_proxy_detector_node")
        self.args = args
        ...
        self.get_logger().info(
            f"YOLO wrist COCO proxy detector started: camera={args.camera}, "
    ...
    def on_timer(self):
        ...
        if not detected and self.args.publish_miss:
            self.publish_status({
                "detected": False,
                "reason": "no_coco_proxy_candidate",
                "stamp": time.time(),
            })
...
def main():
    ...
    rclpy.init()
    node = YoloWristCocoProxyDetector(args)
