#!/usr/bin/env python3
"""Detect COCO sports balls from a ROS Image topic without commanding the robot."""
...
class SportsBallTopicDetector(Node):
    def __init__(self, args) -> None:
        super().__init__("sports_ball_topic_detector")
        self.args = args
        ...
        self.get_logger().info(
            f"Sports ball test: {args.image_topic} -> {args.annotated_topic}, "
    ...
    def on_image(self, msg: Image) -> None:
        ...
        try:
            ...
            if best is not None:
                ...
                self.get_logger().info(
                    f"sports ball detected: confidence={best[0]:.2f}, "
...
def parse_args():
    ...
    parser.add_argument("--detection-topic", default="/sports_ball_detection")
...
def main() -> None:
    args = parse_args()
    rclpy.init()
    node = SportsBallTopicDetector(args)
    try:
        rclpy.spin(node)
