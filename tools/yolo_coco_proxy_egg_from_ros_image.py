#!/usr/bin/env python3
"""ROS Image -> YOLO COCO proxy class -> /egg_detection.

This version is ACT-pipeline safe: it does NOT publish /emergency_stop.
The detector uses a COCO class as a demo-time egg candidate proxy.
"""
...
class CocoProxyEggFromRosImage(Node):
    def __init__(self, args):
        super().__init__("coco_proxy_egg_from_ros_image")
        self.args = args
        ...
        self.get_logger().info(
            f"YOLO COCO proxy egg detector: {args.image_topic} -> {args.output_topic}, "
    ...
    def publish_detection(self, stamp, image_width, image_height, center_x, bottom_y, confidence):
        ...
        if estimate is None:
            self.get_logger().warn("COCO proxy candidate detected, but ground distance estimate was rejected")
            return
        ...
        self.get_logger().info(
            f"egg candidate detected via COCO proxy: conf={confidence:.2f}, bearing={math.degrees(bearing):.1f}deg, "
    ...
    def on_image(self, msg: Image):
        ...
        if self.args.view:
            cv2.imshow("coco_proxy_egg_ros", result.plot())
            cv2.waitKey(1)
...
def main():
    args = parse_args()
    rclpy.init()
    node = CocoProxyEggFromRosImage(args)
    try:
        rclpy.spin(node)
